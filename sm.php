...
// 缓存清除   远程: https://你的域名/sm.php?action=clear_cache&key=hellotv  本机: http://127.0.0.1/sm.php?action=clear_cache
function clearCache() {
...
function showCacheStatus() {
    ...
    if (extension_loaded('apcu')) {
        ...
        try {
            ...
            // 检查频道列表缓存
            $channels = apcu_fetch('smart_channels');
            if ($channels !== false) {
                $output[] = "📡 频道列表缓存: 正常";
                $output[] = "频道数量: " . count($channels);
                
                // 查找缓存过期时间
                if (isset($cacheInfo['cache_list']) && is_array($cacheInfo['cache_list'])) {
                    foreach ($cacheInfo['cache_list'] as $item) {
                        if (isset($item['key']) && $item['key'] === 'smart_channels') {
                            if (isset($item['ttl'], $item['mtime'])) {
                                $timeToExpire = $item['ttl'] - (time() - $item['mtime']);
...
function getChannelList($forceRefresh = false) {
    ...
    if (extension_loaded('apcu')) {
        ...
        error_log("[ChannelList] 频道列表已更新，共 " . count($list) . " 个频道，缓存有效期 " . CONFIG['cache_ttl'] . " 秒");
    }
...
function getCodeMap($forceRefresh = false, $readonly = false) {
    ...
    $channels = $readonly ? (extension_loaded('apcu') ? (apcu_fetch('smart_channels') ?: []) : []) : getChannelList();
    $idToCode = [];
    $codeToId = [];
    $codeExpire = [];
